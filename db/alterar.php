...
if($_GET['codigo']) {// pega o dados informado na url do navegador do usuario no chave codigo
    $sql = "SELECT * FROM cadastro WHERE id = ?";
    $stmt = $conexao->prepare($sql);// usa prepare para validação e não permitir sql injections
    $stmt->bind_param("i", $_GET['codigo']);//pega o paramentro i= inteiro do get e seta para o paramentro mencionado no sql acima (id = ?)
    
    if($stmt->execute()) {// se for executado  com veerdadeiro
...
if(count($_POST) > 0) {
    ...
    if(!count($erros)) {
        ...
        site = ?, filhos = ?, salario = ?
        WHERE id = ?"; // o id precisa ser carregado pelo html abaixo <form action="#" method="post    <input type="hidden" name="id" value="<?= $dados['id']
        ...
        $params = [
            ...
            $dados['site'],
            $dados['filhos'],
            $dados['salario'] ? str_replace(",", ".", $dados['salario']) : null,// alterar para separado com virgula ao invés de ponto, no programa ele acieta valor nulo
            $dados['id'],
        ];

        $stmt->bind_param("ssssidi", ...$params);// prepara os dados para inserção , necessário informar o tipo de cada dado, s = string, i = inteiro e d = decimal
